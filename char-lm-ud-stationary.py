...
def prepareDataset(data):
      ...
      for sentence in data:
         for word in sentence:
             for char in word["word"]:
                numeric[-1].append(stoi[char]+2 if random.random() > args.char_noise_prob else 2+random.randint(0, len(itos)-1))
...
#          out = dropout(out)
 ...
 #     print(target_tensor)
      ...
      if printHere:
         lossTensor = print_loss(log_probs.view(-1, len(itos)+2), target_tensor.view(-1)).view(args.sequence_length, len(numeric))
...
#      setattr(rnn, name, oldParameters[name])
   ...
   numericAndBoundaries = prepareDataset(dev_data)
