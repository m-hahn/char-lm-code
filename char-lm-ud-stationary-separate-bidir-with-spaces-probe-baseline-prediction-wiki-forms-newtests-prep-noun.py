...
preposition = "mit"
prefix = "siegingen"
adj = "sehrextremunglaublichklein"

for i in range(1,1000):
    femDat = random.choice(feminineDatives)
    mascNom = random.choice(masculineNominatives)

    plurDat = random.choice(pluralDatives)
    mascAcc = random.choice(masculineAccusatives)

    correctDer += (1 if 0 == doChoiceList([f'.{prefix}{preposition}der{adj}en{femDat}.', f'.{prefix}{preposition}der{adj}e{mascNom}.'], printHere=True) else 0)
    correctDen += (1 if 0 == doChoiceList([f'.{prefix}{preposition}den{adj}en{plurDat}.', f'.{prefix}{preposition}den{adj}en{mascAcc}.'], printHere=True) else 0)
    print("WITH ADJECTIVE", correctDer/i, correctDen/i)

    correctNoAdjDer += (1 if 0 == doChoiceList([f'.{prefix}{preposition}der{femDat}.', f'.{prefix}{preposition}der{mascNom}.'], printHere=True) else 0)
    correctNoAdjDen += (1 if 0 == doChoiceList([f'.{prefix}{preposition}den{plurDat}.', f'.{prefix}{preposition}den{mascAcc}.'], printHere=True) else 0)
    print("NO ADJECTIVE", correctNoAdjDer/i, correctNoAdjDen/i)

    correctControlDer += (1 if 1 == doChoiceList([f'.dasproblemwarder{adj}en{femDat}.', f'.dasproblemwarder{adj}e{mascNom}.'], printHere=True) else 0)
    correctControlDen += (1 if 1 == doChoiceList([f'.siesahenden{adj}en{plurDat}.', f'.siesahenden{adj}en{mascAcc}.'], printHere=True) else 0)
    print("CONTROL", correctControlDer/i, correctControlDen/i)
