...
rnn_parameter_names = [name for name, _ in rnn_forward.named_parameters()]
...
output_forward = torch.nn.Linear(args.hidden_dim, len(itos)+3).cuda()
output_backward = torch.nn.Linear(args.hidden_dim, len(itos)+3).cuda()
...
modules = [rnn_forward, rnn_backward, output_forward, output_backward, char_embeddings]
def parameters():
...
named_modules = {"rnn_backward" : rnn_backward, "rnn_backward" : rnn_backward, "output_backward" : output_backward, "output_backward" : output_backward, "char_embeddings" : char_embeddings, "optim" : optim}
...
def forward(numericAndBoundaries, train=True, printHere=False):
     # print(numeric)
      ...
      input_tensor_forward = Variable(torch.LongTensor(numeric).transpose(0,1)[1:-1].cuda(), requires_grad=False)
      input_tensor_backward = Variable(torch.LongTensor([x[::-1] for x in numeric]).transpose(0,1)[1:-1].cuda(), requires_grad=False)
...
#      target_tensor = torch.cat([target_tensor_forward, target_tensor_backward], dim=2)
    ...
    #  print(char_embeddings)
      ...
      if train:
         embedded_forward = char_dropout(embedded_forward)
      embedded_backward = char_embeddings(input_tensor_backward)
      if train:
         embedded_backward = char_dropout(embedded_backward)


      out_forward, _ = rnn_forward_drop(embedded_forward, None)
      out_forward = out_forward.view(args.sequence_length+1, len(numeric), -1)
      logits_forward = output_forward(out_forward) 
      log_probs_forward = logsoftmax(logits_forward)
      loss = train_loss(log_probs_forward.view(-1, len(itos)+3), target_tensor_forward.view(-1))

      out_backward, _ = rnn_backward_drop(embedded_backward, None)
      out_backward = out_backward.view(args.sequence_length+1, len(numeric), -1)
      logits_backward = output_backward(out_backward) 
      log_probs_backward = logsoftmax(logits_backward)
      loss += train_loss(log_probs_backward.view(-1, len(itos)+3), target_tensor_backward.view(-1))


      if printHere:
         lossTensor = print_loss(log_probs_forward.view(-1, len(itos)+3), target_tensor_forward.view(-1)).view(args.sequence_length+1, len(numeric))
         ...
         for i in range((args.sequence_length-1)-1):
            if boundaries_index[0] < len(boundaries[0]) and i+1 == boundaries[0][boundaries_index[0]]:
               ...
               boundary = False
            print((losses[i][0], losses[i][0], itos[numeric[0][i+1]-3], boundary))
      return loss, 2 * len(numeric) * args.sequence_length
