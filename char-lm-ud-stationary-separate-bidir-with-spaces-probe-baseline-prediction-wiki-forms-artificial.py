...
for i in range(1, 100):
    ...
    choiceSg = doChoiceList([f".die{noun}sindgut.", f".der{noun}sindgut.", f".das{noun}sindgut."], printHere=True) # if we replace "sind" with "ist", the pattern changes massively (these 'odd' words are apparently preferred to be neuters)
    choice_en = doChoiceList([f".die{noun}ensindgut.", f".der{noun}ensindgut.", f".das{noun}ensindgut."], printHere=True)
    choice_verb = doChoiceList([f".die{noun}istgut.", f".die{noun}sindgut."], printHere=True) # this one doesn't care about the adjective ending very much, the ending of the noun is more important
    choice_verb_pl = doChoiceList([f".die{noun}enistgut.", f".die{noun}ensindgut."], printHere=True) # this one doesn't care about the adjective ending very much, the ending of the noun is more important
