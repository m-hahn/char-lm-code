...
predictor2 = [predictor[i] for i in range(len(predictor))]
...
plt.errorbar(range(-3, 4), [averagePMI1[index.index(j)] for j in range(-3,4)], yerr=yerr, color="blue", linewidth=4.0)
...
plt.plot(range(-3, 4), [averagePMI2[index.index(j)] for j in range(-3,4)], color="red", linewidth=4.0)
#plt.legend()
plt.show()
plt.savefig("/checkpoint/mhahn/segmentation-profile-flattened-pmis-"+args.language+"-ci.pdf", bbox_inches='tight')
plt.close()
...
replicateMeans = [[None for _ in range(boot)] for _ in range(-3, 4)]
for w in range(boot):
  replicateInd = [random.randint(0, len(surprisals1)-1) for _ in surprisals1]
  replicate = surprisals1[replicateInd,:]
  for q in range(-3, 4):
     replicateMeans[q][w] = replicate[:,q].mean(axis=0)
for q in range(-3, 4):
   print(q)
   ...
   lower = replicateMeans[q][int(0.05*boot)]
   upper = replicateMeans[q][int(0.95*boot)]
   yerr[0][q] = averageSurprisal1[q] - lower
   yerr[1][q] = upper - averageSurprisal1[q]
plt.errorbar(range(-3, 4), [averageSurprisal1[index.index(j)] for j in range(-3,4)], yerr=yerr, color="blue", linewidth=4.0)




#averageSurprisal1 = surprisals1.mean(axis=0)
#plt.plot(range(-3, 4), [averageSurprisal1[index.index(j)] for j in range(-3,4)], label=name, color="blue", linewidth=4.0)
averageSurprisal2 = surprisals2.mean(axis=0)
plt.plot(range(-3, 4), [averageSurprisal2[index.index(j)] for j in range(-3,4)], color="red", linewidth=4.0)
#plt.legend()
plt.show()
plt.savefig("/checkpoint/mhahn/segmentation-profile-flattened-surprisals-"+args.language+"-ci.pdf", bbox_inches='tight')
plt.close()
...
replicateMeans = [[None for _ in range(boot)] for _ in range(-3, 4)]
for w in range(boot):
  replicateInd = [random.randint(0, len(entropies1)-1) for _ in entropies1]
  replicate = entropies1[replicateInd,:]
  for q in range(-3, 4):
     replicateMeans[q][w] = replicate[:,q].mean(axis=0)
for q in range(-3, 4):
   print(q)
   ...
   lower = replicateMeans[q][int(0.05*boot)]
   upper = replicateMeans[q][int(0.95*boot)]
   yerr[0][q] = averageEntropy1[q] - lower
   yerr[1][q] = upper - averageEntropy1[q]
plt.errorbar(range(-3, 4), [averageEntropy1[index.index(j)] for j in range(-3,4)], yerr=yerr, color="blue", linewidth=4.0)



#averageEntropy1 = entropies1.mean(axis=0)
#plt.plot(range(-3, 4), [averageEntropy1[index.index(j)] for j in range(-3,4)], label=name, color="blue", linewidth=4.0)
averageEntropy2 = entropies2.mean(axis=0)
plt.plot(range(-3, 4), [averageEntropy2[index.index(j)] for j in range(-3,4)], color="red", linewidth=4.0)
#plt.legend()
plt.show()
plt.savefig("/checkpoint/mhahn/segmentation-profile-flattened-entropies-"+args.language+"-ci.pdf", bbox_inches='tight')
plt.close()
