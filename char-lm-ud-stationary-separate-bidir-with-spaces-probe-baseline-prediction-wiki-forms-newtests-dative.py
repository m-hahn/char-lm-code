...
for dative in datives:
   dative = dative.replace(" ","")
   resultDat += 1 if 0 == doChoiceList(["."+left+objects[0]+middle+dative+".", "."+left+objects[1]+middle+dative+"."], printHere=True) else 0
   counter += 1
   print(resultDat/counter)


with open("accusatives.txt", "r") as inFile:
   accusatives = inFile.read().strip().split("\n")[1:]
resultAcc = 0
counterAcc = 0
for accusative in accusatives:
   accusative = accusative.replace(" ","")
   resultAcc += 1 if 1 == doChoiceList(["."+left+objects[0]+middle+accusative+".", "."+left+objects[1]+middle+accusative+"."], printHere=True) else 0
   counterAcc += 1
   print(resultAcc/counterAcc)

print("..")
print(counter, resultDat/counter)
print(counterAcc, resultAcc/counterAcc)
