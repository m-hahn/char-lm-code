...
def encodeListOfWords(words):
    ...
    hidden_forward = None
    hidden_states = []
    for i in range(len(input_cut)):
       _, hidden_forward = rnn_drop(embedded_forward[i].unsqueeze(0), hidden_forward)
       hidden_states.append(hidden_forward[0])
#    print(embedded_forward.size())
#    print(words)
#    print(input_cut)
...
#    hidden = torch.mean(hidden_states, dim=0)
    hidden, _ = torch.max(hidden_states, dim=0)
...
with open("/private/home/mhahn/data/similarity/SentEval/data/probing/odd_man_out.txt", "r") as inFile:
  somo = [x.split("\t") for x in inFile.read().strip().lower().split("\n")]
somo_train = [x for x in somo if x[0] == "tr"] #[:5000]
somo_dev = [x for x in somo if x[0] == "va"]
somo_test = [x for x in somo if x[0] == "te"]

train_encoded = []
for i in range(len(somo_train)):
   if i % 50 == 0:
     print(i, float(i)/len(somo_train))
   x = somo_train[i]
   train_encoded.append(encodeListOfWords([x[2].replace(" ","")])[0])
train_dependent = [1 if x[1] == "c" else 0 for x in somo_train]

from sklearn.model_selection import train_test_split
x_train, x_test, y_train, y_test = train_test_split(train_encoded, train_dependent, test_size=0.1, random_state=0, shuffle=True)
