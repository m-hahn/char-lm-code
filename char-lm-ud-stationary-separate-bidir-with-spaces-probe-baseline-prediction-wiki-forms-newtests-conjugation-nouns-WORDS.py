...
with open("germanConjugationCleaned.txt", "r") as inFile:
    ...
    for verb in conj:
        ...
        nounS = "_NONE_"
        nounP = "_NONE_"
        while nounS not in stoi or nounP not in stoi or len(nounP) == 1 or len(nounS) == 1:
        ...
        if not all([x in stoi for x in praesens[2][0][0][1:]]):
           continue
        if not all([x in stoi for x in praesens[5][0][0][1:]]):
           continue
