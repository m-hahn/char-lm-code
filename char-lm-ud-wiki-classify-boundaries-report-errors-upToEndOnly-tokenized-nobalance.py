# python char-lm-ud-wiki-classify-boundaries-report-errors-upToEndOnly-tokenized-nobalance.py --language english  --batchSize 128 --char_dropout_prob 0.001 --char_embedding_size 200 --char_noise_prob 0.0 --hidden_dim 1024 --language english --layer_num 3 --learning_rate 3.6  --myID 282506230 --load-from wiki-english-nospaces-bptt-282506230 --sequence_length 80 --weight_dropout_hidden 0.01 --weight_dropout_in 0.0

# python char-lm-ud-wiki-classify-boundaries-report-errors-upToEndOnly-tokenized-nobalance.py --language german --batchSize 128 --char_embedding_size 100 --hidden_dim 1024 --layer_num 2 --weight_dropout_in 0.1 --weight_dropout_hidden 0.2 --char_dropout_prob 0.0 --char_noise_prob 0.01 --learning_rate 0.2 --load-from wiki-german-nospaces-bptt-910515909

# python char-lm-ud-wiki-classify-boundaries-report-errors-upToEndOnly-tokenized-nobalance.py --language italian --batchSize 128 --char_embedding_size 200 --hidden_dim 1024 --layer_num 2 --weight_dropout_in 0.1 --weight_dropout_hidden 0.2 --char_dropout_prob 0.0 --char_noise_prob 0.01 --learning_rate 0.2 --load-from wiki-italian-nospaces-bptt-855947412
...
def forward(numeric, train=True, printHere=False, enforceBalancing=True):
      ...
      for i in range(len(boundaries)): # for each batch sample
         ...
         true = sum([((x == None) if target == False else (x is not None and x not in wordsSoFar)) for x in boundaries[i][int(args.sequence_length/2):-10]]) # candidates
         if true == 0:
            continue
         soFar = 0
         for j in range(int(len(boundaries[i])/2), len(boundaries[i])-10):
           if (lambda x:((x is None if target == False else (x is not None and x not in wordsSoFar))))(boundaries[i][j]):
...
#                  print(target, relevantWords[-1], relevantNextWords[-1])
                  ...
                  labels_sum += labels[-1]
                  if target and enforceBalancing:
...
with open(f"results/segmentation-{args.language}-frequent-errors.txt", "w") as outFile:
