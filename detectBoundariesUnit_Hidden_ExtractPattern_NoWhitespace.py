# python detectBoundariesUnit_Hidden_ExtractPattern_NoWhitespace.py --language english  --batchSize 128 --char_dropout_prob 0.001 --char_embedding_size 200 --char_noise_prob 0.0 --hidden_dim 1024 --language english --layer_num 3 --learning_rate 3.6  --myID 282506230 --load-from wiki-english-nospaces-bptt-282506230 --weight_dropout_hidden 0.01 --weight_dropout_in 0.0

# python detectBoundariesUnit_Hidden_ExtractPattern_NoWhitespace.py --batchSize 128 --char_dropout_prob 0.001 --char_embedding_size 100 --char_noise_prob 0.0 --hidden_dim 1024 --language german --layer_num 2 --learning_rate 2.0 --weight_dropout_hidden 0.05 --weight_dropout_in 0.01 --load-from wiki-german-nospaces-bptt-910515909
...
#      print(embedded.size())
      ...
      with open(f"{LOG_HOME}/boundary-neuron/{args.language}_{args.load_from}_{counter}.txt", "w") as outFile:
         for i in range(len(embedded)):
            ...
            print(" ".join([str(x) for x in ([i, itos[numeric[0][i]-3]] + [float(hidden[1][:,0,:].flatten()[x]) for x in neuron] + [ boundaries[0][i+1]])]), file=outFile) # , "*"*(int(10*(1+round(float(hidden[1][:,0,:].flatten()[neuron]),3))))
...
if True:
   training_data = corpusIteratorWiki.dev(args.language)
   print("Got data")
