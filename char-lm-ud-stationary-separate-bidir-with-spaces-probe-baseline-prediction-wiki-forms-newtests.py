...
for sentence in training.iterator():
  for line in sentence:
     if line["posUni"] == "NOUN" and "Number=Plur" in line["morph"]:
       if line["lemma"] == line["word"] and line["word"].endswith("er") and "Gender=Fem" not in line["morph"]:
        if line["lemma"] not in singularsEqualToPlurals:
          gender = [x for x in line["morph"] if x.startswith("Gender=")]
          if len(gender) != 1:
             print(line)
             continue
          gender = gender[0][7:]
          singularsEqualToPlurals[line["word"]] = gender
          print(line["lemma"], gender)
          noun = line["word"]
          article = {"Masc" : "der", "Neut" : "das"}[gender]
          forSingSum += 1 if 0 == doChoiceList(["."+article+noun+"istgut.", "."+article+noun+"sindgut."], printHere=True) else 0
          forPlurSum += 1 if 1 == doChoiceList(["."+"die"+noun+"istgut.", "."+"die"+noun+"sindgut."], printHere=True) else 0
          counter += 1
