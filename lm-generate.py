...
for i in range(1000):
   hidden = None
   sampled = 0 #random.randint(0, len(itos)-1)
#   print("")
   outputString = ""
   for j in range(40):
#      print(sampled)
      input_tensor = Variable(torch.LongTensor([sampled]).view(1,1).cuda(), requires_grad=False)

      embedded = char_embeddings(input_tensor)

      out, hidden = rnn(embedded, hidden)
      logits = output(out)
      probs = softmax(logits)
      entropy = - (probs * torch.log(probs)).sum().cpu().data.numpy()[0]
      if len(outputString) > 1 and entropy > 2.2:
          break
      #print(entropy)
      sampled = numpy.random.choice(a=(len(itos)+1), p=probs.data.cpu().view(len(itos)+1).numpy())
      outputString += itos[sampled-1]
   print(outputString)
quit()
