...
alphabet = {"german" :  "abcdefghijklmnopqrstuvwxyzßäöü",
            "english" : "abcdefghijklmnopqrstuvwxyz",
            "italian" : "abcdefghijklmnopqrstuvwxyzàèéìòù"}[args.language]
...
alphabetIndices = [i for i in range(len(itos)) if itos[i] in alphabet]

embeddings = [embeddings[i+3] for i in alphabetIndices]
alphabetNames = [itos[i] for i in alphabetIndices]
...
print(y)
for i, char in enumerate(alphabetNames):
   print("\t".join([char, str(y[i])]))
...
plt.show()
plt.savefig(FIGURES_HOME+"/figures/char-emb-clustering-output_output-phonetic-"+args.load_from+".pdf", bbox_inches='tight')
plt.close()
