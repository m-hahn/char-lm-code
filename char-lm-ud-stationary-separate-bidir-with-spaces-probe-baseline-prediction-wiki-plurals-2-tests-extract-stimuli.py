# This scripts generates the German plural stimuli.
...
for sentence in training.iterator():
 for line in sentence:
   if line["posUni"] == "NOUN":
      morph = line["morph"]
      if "Number=Plur" in  morph and "Case=Dat" not in morph and len([x for x in morph if x.startswith("Case=")]) > 0:
...
for groupName, group in formations.items(): 
 with open(f"stimuli/german-plurals-{groupName}.txt", "w") as outFile:
   for singular, plural in sorted(list(group)):
