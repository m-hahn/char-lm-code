# python char-lm-ud-wiki-classify-boundaries-report-errors-upToEndOnly-lexCountBaseline-tokenized-nobalance.py --language english
...
#          out = dropout(out)
      ...
      for i in range(len(boundaries)): # for each batch sample
         ...
         true = sum([((x == None) if target == False else (x is not None and x not in wordsSoFar)) for x in boundaries[i][int(args.sequence_length/2):-10]]) # condidates
 #        print(target, true)
         if true == 0:
            continue
         soFar = 0
#         print(list(zip(boundaries[i], boundariesAll[i])))
         for j in range(len(boundaries[i])-10):
